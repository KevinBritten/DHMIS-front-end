  ...
  useEffect(() => {
    ...
    const fetchOtherCharacters = async () => {
      ...
      async function pushTransformedCharacters(address) {
        const txnOtherCharacter = await gameContract.getOneTokenByAddress(
          address
        );
      ...
      const otherCharactersArray = await Promise.all(
        otherPlayerAddresses.map(pushTransformedCharacters)
      );
    ...
    const onAttackComplete = async (
       ...
       */
      const isCriticalHitMessage = isCriticalHit
        ? "Critical Hit! You did double damage. "
        : "";
      const bossDeadMessage = bossDead
        ? `You killed roy and got the eth! Roy has come back even stronger, now with ${bossHp}hp.`
        : "";
      const playerDeadMessage =
        newPlayerHp == 0
          ? "Oh no, Roy killed you! You can get back in the game for 0.125eth."
          : "";
      const attackMessage =
        bossDead || newPlayerHp == 0
          ? bossDeadMessage + " " + playerDeadMessage
          : "You hit Roy, and got hit back.";
  ...
  const runHealAction = async () => {
    try {
      if (gameContract) {
        ...
        console.log("Healing...");
        const attackTxn = await gameContract.restoreHealth({
    ...
    <div className="arena-container">
          ...
          <div className="player">
            <div className="image-content">
              ...
              <div className="health-bar">
                ...
                {characterNFT.hp == 0 && (
                  <button className="heal-btn" onClick={runHealAction}>
                    {healState == "healing"
                      ? "Healing!"
                      : "Click to heal for 0.125eth"}
                  </button>
                )}
              </div>
            </div>
      ...
      {otherCharacters && (
        <div className="other-players-container">
          ...
          <div className="other-players-inner-container">
            {otherCharacters.map((character, i) => {
              return (
                <div className="other-player" key={i}>
                  ...
                  <a
                    ...
                    href={`https://rinkeby.etherscan.io/address/${character.address}`}
                  >{`${character.address.slice(0, 12)}...`}</a>
      ...
      {siteMessage.length > 1 && (
        <div className="site-message">
          <p>{siteMessage}</p>
          <button
            className="site-message__close-btn"
            onClick={closeSiteMessage}
          >
            X
          </button>
        </div>
